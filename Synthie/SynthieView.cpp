...
CSynthieView::CSynthieView()
{
	...
	mSynthesizer.SetNumChannels(NumChannels());
	mSynthesizer.SetSampleRate(SampleRate());
}
...
void CSynthieView::OnGenerateSynthesizer()
{
	...
	while (mSynthesizer.Generate(frame))
	{
...
void CSynthieView::OnFileOpenscore()
{
	...
	CFileDialog dlg(TRUE, L".score", nullptr, 0, szFilter, nullptr);
	if (dlg.DoModal() != IDOK) return;

	auto pathName = dlg.GetPathName();
	mSynthesizer.OpenScore(pathName);
}
